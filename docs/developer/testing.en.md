...
### Backend (Rust) Exemption Register

After the Rust unit-test pass (batch 1-5): full suite region 82.00% / line 78.68% (430 tests). Core-domain branch coverage: crash_report 87.53%, preferences 91.76%, recovery 90.94%, redact 100%, types 100%.

The following paths are exempted as objectively unreachable (require real external environment):

| Location                                                                 | Exemption type          | Evidence                                                                                                                                                                                                                                                                          |
| ------------------------------------------------------------------------ | ----------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `commands/notifications.rs` command body (`show`)                        | Objectively unreachable | Signature binds `AppHandle` (Wry); `AppHandle<MockRuntime>` is type-incompatible, and success/failure needs the real notification plugin + OS service. Validation was extracted to `validate_notification` and fully covered                                                      |
| `commands/tray.rs` `init_tray` / `handle_tray_event` / window visibility | Objectively unreachable | Requires a real system tray + Wry window; `tray_quit` calls `app.exit(0)` which panics under MockRuntime (`mock_runtime.rs:144` "not implemented")                                                                                                                                |
| `commands/quick_pane.rs` window show/hide, shortcut registration         | Objectively unreachable | `register_quick_pane_shortcut` needs the global-shortcut plugin + Wry; positioning needs real cursor/monitor. Validation extracted to `validate_shortcut` and covered                                                                                                             |
| `commands/crash_report.rs` `_impl` wrappers / `setup_panic_hook`         | Objectively unreachable | `_impl` needs AppHandle + `spawn_blocking` on real dirs; `setup_panic_hook` mutates the process-global panic hook and would pollute all other tests. Pure logic (`init_consent_from_path`, `persist_consent_to_path`, `escape_json_string`, read/delete helpers) is fully covered |
| `lib.rs` / `main.rs` / `bindings.rs`                                     | Boilerplate (fast path) | Tauri app assembly, plugin registration, and the `run()` event loop need a full desktop environment; `export_ts_bindings` is a manual `#[ignore]` command                                                                                                                         |
| `commands/notifications.rs` mobile branch                                | Objectively unreachable | `#[cfg(mobile)]` does not exist in desktop test compilation                                                                                                                                                                                                                       |
| `utils/paths.rs` two defensive `map_err` closures                        | Objectively unreachable | `app_data_dir()` always succeeds under MockRuntime; `create_dir_all` always succeeds under TempDir                                                                                                                                                                                |
